...
- MVP runner는 실제 variant 파일을 만들지 않는다. 따라서
...
Runner가 파일 복사 직전에 stop 여부를 확인할 때 사용한다.
...
`generated_delta`는 실제 디스크에 생성된 파일 수만 반영한다. MVP runner는
원본 이미지 1장당 복사 파일 1개만 만들기 때문에 성공 시 `1`, 실패 시 `0`이다.

반환 해석:

- `RETURNING status = 'RUNNING'` — count 반영 완료. 다음 이미지로 진행한다.
- `RETURNING status = 'STOPPED'` — stop 요청 중 이미 복사 중이던 파일의 count를
...
4. 이미지 루프: 각 이미지에 대해
   - 복사 직전에 `Q_TASK_STATUS`로 현재 status를 확인한다. `RUNNING`이 아니면
     루프를 break한다.
   ...
   - `RETURNING status = 'STOPPED'`이면 복사 중이던 파일의 count 반영을 마친
...
- `workerCount`를 생략하면 `1`로 저장되고, `2` 이상으로 보내도 MVP runner는
  단일 실행 흐름으로 처리한다.
- `variantsPerImage`가 `2` 이상이어도 MVP runner의 `generatedImageCount`는 실제
  복사 성공 파일 수와 일치한다.
- Runner 실행 중 사용자가 `POST .../stop`을 호출하면 DB의 `status`가 즉시
  `STOPPED`로 전이되고, runner는 복사 중이던 파일의 count를 반영한 뒤 자연
  종료한다.
