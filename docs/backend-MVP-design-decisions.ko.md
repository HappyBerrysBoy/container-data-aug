...
- 같은 시점에 새로 추가된 task 필드(예: `variantsPerImage`)는 `setdefault`로 backfill하여 이전 JSON 상태 파일도 새 스키마로 안전하게 직렬화되도록 합니다.
...
- 구현: `services/project_service.py`의 `rescan_project()`가 `services/folder_scanner.py`의 `scan_folder()`를 재사용하여 새 카운트를 계산하고 `repositories/json_store.py`의 `mutate()`로 in-place 갱신합니다.
...
- `workerCount`: `1` 이상.
- `variantsPerImage`: `1` 이상 `90` 이하. 범위 밖은 `422 VALIDATION_ERROR`.
- `outputFolderName`: 비어 있지 않은 폴더 이름(경로 X). §6 참고.

`variantsPerImage`의 상한 `90`은 단일 작업에서 출력 폴더 크기가 비현실적으로 커지는 것을 막기 위한 안전장치입니다. 향후 운영 데이터가 쌓이면 조정할 수 있습니다. 프론트엔드 옵션 모달은 입력값을 자동으로 `1~90` 범위로 클램프하여 사용자가 잘못된 값을 보내지 않도록 합니다.
